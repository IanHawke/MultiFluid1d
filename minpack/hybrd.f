     ...
     *                 qtf,wa1,wa2,wa3,wa4)
      ...
      real(kind=wp) xtol,epsfcn,factor
      real(kind=wp) x(n),fvec(n),diag(n),fjac(ldfjac,n),r(lr),
...
c     **********
      ...
      integer iwa(1)
      logical jeval,sing
      real(kind=wp) actred,delta,epsmch,fnorm,fnorm1,one,pnorm,
     *                 prered,p1,p5,p001,p0001,ratio,sum,temp,xnorm,
     *                 zero
      real(kind=wp) dpmpar,enorm
...
c
c            write(*,*) 'hybrd pre second', info
            if (delta .le. xtol*xnorm .or. fnorm .eq. zero) info = 1
c            write(*,*) 'hybrd second', info
